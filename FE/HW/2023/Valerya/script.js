const root = document.querySelector('.root'); // эту переменную можно было не создавать 
const form_txt = document.querySelector('.form_txt')
const input_id = document.querySelector('.input_id');
const form = document.querySelector('.form');
const btn = document.querySelector('.btn');

const box = document.querySelector('.box'); // обьявляем контейнер который создали в HTML
...
const showProducts = () => {
    box.innerHTML =''  // перед тем как перерисовать страницу очищаем созданный нами контейнер .box
...
const addProduct = (e) => {
    ...
    const deleteProduct = (e) => {
        ...
        showProducts() // после удаления из локалстореджа запускаем функцию рендера еще раз уже с обновленным массивом в локалсторедж (при этом помним что мы очистили контейнер с продуктами перед новым рендером (строка 45) ) 
    };
